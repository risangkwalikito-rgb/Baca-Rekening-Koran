...
22	4500553842	BCA	BANK BCA PELABUHAN""",
    "Mandiri": """1140073007000	MANDIRI	BANK MANDIRI BAKAUHENI
1090066668831	MANDIRI	BANK MANDIRI CASHLESS BATAM
1070015155197	MANDIRI	BANK MANDIRI PNP SIBOLGA
1630061161165	MANDIRI	BANK MANDIRI MERAK
1430020184444	MANDIRI	BANK MANDIRI KETAPANG
1400022522255	MANDIRI	BANK MANDIRI PNP SURABAYA
1490057589006	MANDIRI	BANK MANDIRI BALIKPAPAN 2
0310015162970	MANDIRI	BANK MANDIRI CASHLESS BATULICIN
1860002546394	MANDIRI	BANK MANDIRI CASHLESS AMBON
1860002703730	MANDIRI	BANK MANDIRI CASHLESS TERNATE
1500017615491	MANDIRI	BANK MANDIRI CASHLESS BITUNG
1810033361610	MANDIRI	BANK MANDIRI CASHLESS KUPANG
1610090006003	MANDIRI	BANK MANDIRI KAYANGAN
1610003595639	MANDIRI	BANK MANDIRI LEMBAR
1610007532349	MANDIRI	BANK MANDIRI CASHLESS SAPE
1700007538485	MANDIRI	BANK MANDIRI CASHLESS BAJOE
1840055405540	MANDIRI	BANK MANDIRI JEPARA
1690003057376	MANDIRI	BANK MANDIRI BANGKA
1740004235792	MANDIRI	BANK MANDIRI SELAYAR""",
    "BNI": "",
}
...
def parse_generic_tabular_file(file_bytes: bytes, filename: str, ext: str) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
    notes: List[str] = []
...
def detect_mandiri_header_row(raw_df: pd.DataFrame) -> int:
    scan_limit = min(len(raw_df), 25)
    best_idx = 11
    best_score = -1

    for idx in range(scan_limit):
        row_texts = [normalize_column_name(v) for v in raw_df.iloc[idx].tolist()]
        score = 0
        joined = " | ".join(row_texts)
        if "posting date" in joined:
            score += 3
        if "remark" in joined:
            score += 2
        if "debit" in joined:
...
def extract_mandiri_account_meta(raw_df: pd.DataFrame, filename: str) -> Tuple[str, str, Optional[float]]:
    account_id = normalize_account_key(Path(filename).stem)
    account_name = ""
    opening_balance: Optional[float] = None

    scan_limit = min(len(raw_df), 15)
    for idx in range(scan_limit):
        row_values = [normalize_spaces(v) for v in raw_df.iloc[idx].tolist()]
        ...
        if re.search(r"\baccount\s*no\b", joined, flags=re.IGNORECASE):
            for value in row_values:
                digits = re.sub(r"\D", "", value)
                if len(digits) >= get_account_length("Mandiri"):
                    account_id = normalize_account_key(digits, length=get_account_length("Mandiri"))
                    cleaned_value = normalize_spaces(value)
                    cleaned_value = re.sub(rf"^{re.escape(account_id)}\s*(?:IDR|USD|SGD)?\s*", "", cleaned_value, flags=re.IGNORECASE)
                    account_name = normalize_spaces(cleaned_value)
                    break

    return account_id, account_name, opening_balance


def parse_mandiri_sheet(
    raw_df: pd.DataFrame,
    filename: str,
    sheet_name: str,
    ...
    if raw_df.empty or raw_df.dropna(axis=0, how="all").dropna(axis=1, how="all").empty:
        manifest_df = pd.DataFrame(
            [
                create_manifest_row(
                    account_id=Path(filename).stem,
                    ...
                    parse_note="Sheet kosong",
                    transaction_count=0,
                )
            ]
        )
        return pd.DataFrame(), manifest_df, [f"{filename} [{sheet_name}]: sheet kosong"]

    account_id, account_name, opening_balance = extract_mandiri_account_meta(raw_df, filename)
    header_row = detect_mandiri_header_row(raw_df)
    data = raw_df.iloc[header_row + 1 :].copy()

    rows: List[Dict[str, object]] = []
    for row_order, (_, row) in enumerate(data.iterrows()):
        ...
        debit = parse_amount(row.iloc[9] if len(row) > 9 else None)
        credit = parse_amount(row.iloc[11] if len(row) > 11 else None)

        balance = None
        if len(row) > 15:
            balance = parse_amount(row.iloc[15])
        ...
        debit_value = float(abs(debit)) if debit is not None else 0.0
        credit_value = float(abs(credit)) if credit is not None else 0.0
        ...
        if balance is None and debit_value == 0 and credit_value == 0 and not description:
            continue
        ...
        rows.append(
            {
                "account_id": account_id,
                "account_name": account_name,
                "trx_date": pd.Timestamp(posting_date).normalize(),
                "description": description,
                "amount": amount,
                "debit": debit_value,
                "credit": credit_value,
        ...
        manifest_df = pd.DataFrame(
            [
                create_manifest_row(
                    ...
                    source_sheet=str(sheet_name),
                    parse_status="NO_TRANSACTION",
                    parse_note="Format Mandiri terbaca, tetapi tidak ada transaksi valid",
                    transaction_count=0,
                )
            ]
        )
        return pd.DataFrame(), manifest_df, [f"{filename} [{sheet_name}]: tidak ada transaksi valid"]

    df = pd.DataFrame(rows)
    manifest_df = build_manifest_from_transactions(df, filename, str(sheet_name))
    notes.append(f"{filename} [{sheet_name}]: Mandiri terbaca | rekening={account_id} | transaksi={len(df)}")
    return df, manifest_df, notes


def parse_mandiri_tabular_file(file_bytes: bytes, filename: str, ext: str) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
    notes: List[str] = []
    results: List[pd.DataFrame] = []
    manifests: List[pd.DataFrame] = []

    if ext == ".csv":
        return parse_generic_tabular_file(file_bytes, filename, ext)
    ...
    for sheet_name, raw_df in workbook.items():
        parsed_df, manifest_df, df_notes = parse_mandiri_sheet(raw_df, filename, str(sheet_name))
        notes.extend(df_notes)
...
def parse_tabular_file(
    ...
    if normalize_spaces(selected_bank).lower() == "mandiri":
        return parse_mandiri_tabular_file(file_bytes, filename, ext)
    return parse_generic_tabular_file(file_bytes, filename, ext)
...
def parse_master_accounts(selected_bank: str) -> pd.DataFrame:
    selected_key = normalize_spaces(selected_bank)
    master_text = DEFAULT_MASTER_TEXT.get(selected_key, "")
    if not normalize_spaces(master_text):
    ...
    for i, uploaded_file in enumerate(uploaded_files, start=1):
        ...
        try:
            if ext == ".pdf":
                ...
                df_file, manifest_file, notes = parse_tabular_file(file_bytes, filename, ext, selected_bank)
            else:
                df_file = pd.DataFrame()
                manifest_file = pd.DataFrame(
                    [
